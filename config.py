...
# user action 列名
USER_ACTION_COLUMNS = ["userid", "feedid", "date_", "device", "read_comment", "comment", "like", "play", "stay",
                       "click_avatar", "forward", "follow", "favorite"]
# feed info 列名
FEED_INFO_COLUMNS = ["feedid", "authorid", "videoplayseconds", "description", "ocr", "asr", "bgm_song_id",
                     "bgm_singer_id", "manual_keyword_list", \
                     "machine_keyword_list", "manual_tag_list", "machine_tag_list", "description_char", "ocr_char",
                     "asr_char"]
# feed drop columns
FEED_DROP_COLUMNS = ['description', 'ocr', 'asr', 'description_char', 'ocr_char', 'asr_char', 'manual_keyword_list',
...
# 各个行为构造训练数据的天数
ACTION_DAY_NUM = {"read_comment": 7, "like": 7, "click_avatar": 7, "forward": 7, "comment": 7, "follow": 7,
                  "favorite": 7}

# 负样本下采样比例((下采样后负样本数/原负样本数))
...
FEA_FEED_LIST = ['feedid', 'authorid',
                 ...
                 'feed_like_sum', 'feed_read_comment_sum', 'feed_click_avatar_sum', 'feed_forward_sum',
                 'feed_comment_sum', 'feed_follow_sum', 'feed_favorite_sum',
                 'feed_like_avg', 'feed_read_comment_avg', 'feed_click_avatar_avg', 'feed_forward_avg',
                 'feed_comment_avg', 'feed_follow_avg', 'feed_favorite_avg',
...
FEA_USER_LIST = ['userid',
                 ...
                 'user_like_sum', 'user_read_comment_sum', 'user_click_avatar_sum', 'user_forward_sum',
                 'user_comment_sum', 'user_follow_sum', 'user_favorite_sum',
                 # 用户各种行为的总数
                 'user_like_avg', 'user_read_comment_avg', 'user_click_avatar_avg', 'user_forward_avg',
                 'user_comment_avg', 'user_follow_avg', 'user_favorite_avg',
                 # 用户各种行为的平均出现次数
